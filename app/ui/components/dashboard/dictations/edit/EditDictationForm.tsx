...
import { User, LANGUAGE_CODES, Dictation } from '@/app/lib/definitions';
import {
  CheckIcon,
  DocumentTextIcon,
  LanguageIcon,
...
import { State, updateDictation } from '@/app/lib/dictation-functions/crud';
...
export default function EditDictationForm({
  ...
  const [state, dispatch] = useFormState<State, FormData>(updateDictationWithId, initialState);
    ...
    <form action={dispatch}>
      <div className="rounded-md bg-gray-50 p-4 md:p-6">
        ...
        <div className="mb-4">
          ...
          <div className="relative mt-2 rounded-md">
            <div className="relative">
              <select defaultValue={dictation.language_code} aria-describedby="language-code-error" name="language_code" id="language_code" className="peer block rounded-md border border-gray-200 py-2 pl-10 text-sm outline-2 placeholder:text-gray-500">
          ...
          <div id="language-code-error" aria-live="polite" aria-atomic="true">
            {state.errors?.language_code &&
              state.errors.language_code.map((error: string) => (
                <p className="mt-2 text-sm text-red-500" key={error}>
                  {error}
                </p>
              ))}
